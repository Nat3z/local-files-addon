...
function logFilenameToUtcDate(filename: string): Date | null {
  ...
  const m = stem.match(/^(\d{4})-(\d{2})-(\d{2})T(\d{2})[-_:](\d{2})[-_:](\d{2})[._-](\d{3})Z$/);
  if (!m) return null;
...
const UMU_RUN_PATH = '/home/' + process.env.USER + '/.local/share/OpenGameInstaller/bin/umu-run/umu-run';
...
const publicKeyNames = fs.readdirSync(join(__dirname, 'keys')).filter(file => file.endsWith('.public.pem')).map(file => file.slice(0, -'.public.pem'.length));

addon.on('configure', (config) => config
  .addStringOption(option =>
    option
      .setName('publicKeyName')
      .setDescription('This is used to encrypt the logs before uploading them to the OGI team or another person for support to ensure others cannot read your logs except the intended recipient.')
...
addon.onTask('upload-latest-log', async (task) => {
  ...
  if (process.platform === 'linux') {
    const input = await task.askForInput('', 'Where is your OpenGameInstaller-Setup.AppImage located?',
      new ConfigurationBuilder()
        .addActionOption(option =>
          option
            .setName('exit')
    ...
    const input = await task.askForInput(' ', 'Select your OpenGameInstaller update folder (the folder that contains the `logs` directory and `latest.log`).',
      new ConfigurationBuilder()
        .addActionOption(option =>
          option
            .setName('exit')
  ...
  while (true) {
    const input = await task.askForInput('Which logs to upload?', 'Please select the logs to upload. Add a log by selecting it below and pressing submit. When you are done, press continue to start the upload.', new ConfigurationBuilder()
      .addStringOption(option =>
        option
          .setName('log')
          ...
          .setDisplayName('Log')
          .setInputType('text')
          .setAllowedValues(logsReadable.map(log => logsToUpload.find(l => l === join(logFolder, logChoiceLabelToFile.get(log) ?? log) || (l.includes(join(updateDirectory, 'latest.log')) && log === 'Latest Log')) ? `✔︎ ${log}` : log))
      )
      .addActionOption(option =>
        option
          .setName('exit')
    ...
    if (!input.continue) {
      if (input.log.startsWith('✔︎ ')) {
        ...
        if (unmarkedLabel === 'Latest Log') {
          logsToUpload = logsToUpload.filter(log => log !== join(updateDirectory, 'latest.log'));
        } else {
          const logFile = logChoiceLabelToFile.get(unmarkedLabel);
          if (logFile) {
            logsToUpload = logsToUpload.filter(log => log !== join(logFolder, logFile));
          }
        }
  ...
  const encryptedFile = await encryptFile(fileContent, addon.config.getStringValue('publicKeyName') ?? 'ogi');
  ...
  const { copyURL } = await task.askForInput('Logs uploaded successfully!', 'The logs have been uploaded successfully. You can now share the following URL with the OGI team to help them diagnose the issue:',
    new ConfigurationBuilder().addStringOption(option =>
      option
        .setName('url')
        .setDescription('The URL of the uploaded logs. You can copy and paste this URL into your browser to view the logs.')
        .setDisplayName('URL')
        .setInputType('text')
        .setDefaultValue(url)
    ).addActionOption(option =>
  ...
  if (copyURL) {
    ...
    addon.notify({
      ...
      type: 'info',
      message: 'URL copied to clipboard. Share this URL with the OGI team to help them diagnose the issue.',
    });
  }
